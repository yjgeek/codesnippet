 ...
 *     },
 *     onError: function (type, errFileData, cutFileDatas, fileDatas) { // 监听选择文件出错事件
 ...
 * fileSelection.files()     // 获取当前所有选择的文件
...
var FileSelection = (function () {
	...
	FileSelection.prototype.$$parseFileData = function (file) {
		...
		file.name.replace(/^(((?:[a-zA-Z]):)?[\\\/]?(?:[\s\S]+?[\\\/])*)([\s\S]+?(\.[\s\S]+?)?)$/, function (all, dirname, drive, basename, extname) {
